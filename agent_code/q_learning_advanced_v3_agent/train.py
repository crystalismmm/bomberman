...
REWARDS = {
    ...
    BAD_BOMB_PLACEMENT: -1.0,  # negative reward for placing a bomb in a useless position
...
def game_events_occurred(
    ...
    if self_action in MOVE_DELTAS and e.CRATE_DESTROYED not in events:
        ...
        if not old_in_danger and not new_in_danger:
            _, _, old_bombing_score = best_safe_bombing_position(
                old_game_state
            )
            _, _, new_bombing_score = best_safe_bombing_position(
                new_game_state
            )

            if not np.isclose(old_bombing_score, new_bombing_score):
