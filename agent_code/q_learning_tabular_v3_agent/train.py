"""Training callbacks for the compact tabular Q-learning agent."""
...
def setup_training(self):
    ...
    # Defensive initialization keeps the callbacks robust if setup order or a
    # manually constructed test namespace omits history attributes.
...
def add_transition_events(
    ...
    """Add compact, state-aligned shaping events for one transition."""
    analysis = analyze_state(
        old_game_state,
        getattr(self, "state_previous_action", None),
    )
    ...
    if e.BOMB_DROPPED in events:
        ...
        # ATTACK bombs deliberately receive no artificial placement reward.
        # A successful attack is learned from KILLED_OPPONENT later on, while
        # avoiding the contradictory USELESS_BOMB penalty from V2.
    ...
    if (
        ...
        if (
            not np.isfinite(old_danger[old_position])
            and not np.isfinite(new_danger[new_position])
        ):
    ...
    if (
        ...
        or target_mode not in (TARGET_COIN, TARGET_BOMBING)
    ):
        return
    ...
    new_danger = danger_time_map(new_game_state)
    if (
        np.isfinite(old_danger[old_position])
        or np.isfinite(new_danger[new_position])
    ):
        return

    old_distance = shortest_path_distance(
        old_game_state,
        old_position,
        analysis.target_position,
    )
    new_distance = shortest_path_distance(
        old_game_state,
        new_position,
        analysis.target_position,
    )
...
def table_entry(
    ...
    """Return a writable entry, creating an unseen state when necessary."""
    if state_key not in q_table:
        q_table[state_key] = np.zeros(len(ACTIONS), dtype=np.float32)
...
def update_q_table(
    ...
    old_values = table_entry(self.q_table, old_key)
    old_q_value = float(old_values[action_index])
    ...
    if new_game_state is None:
        ...
        new_values = q_values_for_state(self.q_table, new_key)
        # All actions remain candidates in the TD target as well as in act().
        max_future_q_value = float(np.max(new_values))

    td_target = reward + DISCOUNT_FACTOR * max_future_q_value
    td_error = td_target - old_q_value
    old_values[action_index] += LEARNING_RATE * td_error

    self.logger.debug(
        "Updated state %s, action %s: old Q = %.3f, new Q = %.3f, "
        "reward = %.3f, future Q = %.3f, TD error = %.3f",
        old_key,
        action,
        old_q_value,
        old_values[action_index],
        reward,
        max_future_q_value,
        td_error,
    )
...
def save_q_table(q_table: dict[StateKey, np.ndarray], path: Path) -> None:
    """Atomically persist the Q-table and its compatibility metadata."""
    payload = {
