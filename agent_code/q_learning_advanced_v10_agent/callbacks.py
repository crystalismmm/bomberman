...
# 1 agent is in danger + 4 directions to nearest safe tile + 1 safe and useful bomb here + 1 bomb quality here +
# 4 directions to the best safe bombing position + 1 whether waiting is necessary +
# 1 whether at least one safe move exists + 1 score of the best safe bombing
# position + 4 route-preserving movement actions + 1 bad-bomb indicator
N_FEATURES = 32
...
def action_preserves_escape_route(
    game_state: dict,
    action: str,
) -> bool:
    """Return whether an action leaves a time-valid route to safety."""
    field = game_state["field"]
    danger_map = danger_time_map(game_state)
    ...
    inside_field = (
        0 <= new_position[0] < field.shape[0]
        and 0 <= new_position[1] < field.shape[1]
    )

    if not inside_field or field[new_position] != 0:
        return False

    # WAIT is executable while the agent is standing on its own bomb.
    remains_on_current_position = (
        action == "WAIT" and new_position == (x, y)
    )

    if (
        new_position in blocked_positions
        and not remains_on_current_position
    ):
        return False

    # The first action happens before bombs are updated. A tile with timer 0
    # explodes immediately afterwards; timer 1 is still enterable now.
    if danger_map[new_position] < 1:
        return False
    ...
    while queue:
        ...
        for dx, dy in MOVE_DELTAS.values():
            ...
            if (
                ...
                and neighbor not in blocked_positions
                and neighbor not in visited
                and danger_map[neighbor] >= neighbor_arrival_time
            ):
                visited.add(neighbor)
                queue.append(
                    (neighbor, neighbor_arrival_time)
                )
...
def state_to_features(game_state: dict) -> np.ndarray | None:
    ...
    # Classify the currently available bomb action unambiguously. A bomb is
    # good only if it is both useful and escapable.
    bomb_quality = bomb_quality_at_position(
        game_state,
        (x, y),
    )
    bomb_is_good = bool(
        bombs_left and bomb_quality > 0.0
    )

    features[18] = float(bomb_is_good)
    features[19] = bomb_quality
    ...
    wait_is_safe = action_preserves_escape_route(
        game_state,
        action="WAIT",
    )
    safe_move_exists = False

    for idx, move in enumerate(MOVE_DELTAS):
        if action_preserves_escape_route(
            game_state,
            action=move,
        ):
            features[27 + idx] = 1.0
    ...
    wait_is_necessary = (
        wait_is_safe and not safe_move_exists
    )

    features[24] = float(wait_is_necessary)
    features[25] = float(safe_move_exists)
    features[26] = bombing_score

    # Every available bomb is classified as either good or bad. This prevents
    # a safe but useless bomb from activating the positive bomb feature.
    features[31] = float(
        bombs_left and not bomb_is_good
    )
