...
OPPONENT_RELEVANCE_THRESHOLD = 0.35
MIN_NASH_TOTAL_VISITS = 24
MIN_NASH_COVERAGE = 0.40
...
def setup(self):
    """Initialize history and load a previous v4 model when available."""
    self.rng = np.random.default_rng(RNG_SEED)
    self.q_table: dict[StateKey, np.ndarray] = {}
    self.visit_table: dict[StateKey, np.ndarray] = {}
    self.solo_q_table: dict[StateKey, np.ndarray] = {}
    self.solo_visit_table: dict[StateKey, np.ndarray] = {}
    ...
    if not MODEL_FILE.is_file():
        ...
        self.logger.warning("No v5 Nash Q-table found at %s. Starting %s untrained.", MODEL_FILE, mode)
        return

    payload = load_model(MODEL_FILE)

    self.q_table = payload["q_table"]
    self.visit_table = payload["visit_table"]
    self.solo_q_table = payload["solo_q_table"]
    self.solo_visit_table = payload["solo_visit_table"]
    self.training_rounds = payload["training_rounds"]

    self.logger.info(
        "Loaded %d joint states and %d solo states after %d training rounds from %s.",
        len(self.q_table),
...
def act(self, game_state: dict) -> str:
    """Choose from the unrestricted action space using learned policies."""
    ...
    allowed = available_actions(game_state)
    opponent, opponent_relevance = most_relevant_opponent_with_score(game_state)
    ...
    (
        ...
        equilibrium_value,
        policy_mode,
    ) = policy_for_state(self, game_state, analysis, opponent, opponent_relevance)
    ...
    self.logger.debug(
        ...
        "Opponent: %s | Relevance: %.3f | Allowed: %s | "
...
def most_relevant_opponent_with_score(game_state: dict):
    """Return the closest credible opponent using one shared BFS."""
    if not game_state["others"]:
        return None, 0.0

    field = game_state["field"]
    own_position = game_state["self"][3]
    distances = bfs_distance_map(field, own_position)
    candidates = []

    for other in game_state["others"]:
        distance = distances.get(other[3])
        distance_score = 0.0 if distance is None else 0.8 ** distance
        direct_bomb_threat = bool(other[2]) and own_position in get_blast_tiles(field, other[3])
        relevance = max(float(direct_bomb_threat), distance_score)
        candidates.append((other, relevance))

    return max(candidates, key=lambda item: item[1])


def most_relevant_opponent(game_state: dict):
    """Compatibility helper returning only the selected opponent."""
    opponent, _ = most_relevant_opponent_with_score(game_state)

    return opponent


def relevant_opponent(game_state: dict):
    """Return an opponent only when its estimated interaction is meaningful."""
    opponent, relevance = most_relevant_opponent_with_score(game_state)

    if opponent is None or relevance < OPPONENT_RELEVANCE_THRESHOLD:
        return None, relevance

    return opponent, relevance


def policy_for_state(self, game_state: dict, analysis: StateAnalysis, opponent=None, relevance: float | None = None):
    """Build a policy over all six actions without a safety mask."""
    own_actions = available_actions(game_state)
    own_indices = np.array([ACTIONS.index(action) for action in own_actions], dtype=int)

    solo_values = vector_for_state(self.solo_q_table, analysis.key, np.float32)
    solo_visits = vector_for_state(self.solo_visit_table, analysis.key, np.int32)

    solo_strategy, solo_value, solo_mode = solve_solo_action_policy(
        solo_values,
        solo_visits,
        analysis.key,
        own_indices,
    ...
    if opponent is None or relevance is None:
        opponent, relevance = most_relevant_opponent_with_score(game_state)

    if opponent is None or relevance < OPPONENT_RELEVANCE_THRESHOLD:
        opponent_strategy = np.zeros(len(ACTIONS), dtype=np.float32)
        return solo_strategy, opponent_strategy, solo_value, solo_mode

    opponent_state = game_state_for_actor(game_state, opponent)
    opponent_actions = available_actions(opponent_state)
    opponent_indices = np.array([ACTIONS.index(action) for action in opponent_actions], dtype=int)

    matrix = q_matrix_for_state(self.q_table, analysis.key)
    visits = q_matrix_for_state(self.visit_table, analysis.key, dtype=np.int32)

    submatrix = matrix[np.ix_(own_indices, opponent_indices)]
    subvisits = visits[np.ix_(own_indices, opponent_indices)]

    if not nash_matrix_is_ready(subvisits):
        opponent_strategy = np.zeros(len(ACTIONS), dtype=np.float32)
        opponent_strategy[opponent_indices] = 1.0 / len(opponent_indices)
        return solo_strategy, opponent_strategy, solo_value, "solo-sparse"

    completed_matrix = complete_sparse_matrix(submatrix, subvisits)
    row_substrategy, column_substrategy, value = solve_zero_sum_game(completed_matrix)

    own_strategy = np.zeros(len(ACTIONS), dtype=np.float32)
    opponent_strategy = np.zeros(len(ACTIONS), dtype=np.float32)
    own_strategy[own_indices] = row_substrategy
    opponent_strategy[opponent_indices] = column_substrategy

    return own_strategy, opponent_strategy, value, "nash"


def vector_for_state(table: dict[StateKey, np.ndarray], state_key: StateKey, dtype) -> np.ndarray:
    """Return a six-action vector without inserting an entry into the table."""
    values = table.get(state_key)

    if values is None:
        return np.zeros(len(ACTIONS), dtype=dtype)

    return np.asarray(values, dtype=dtype)


def fallback_strategy(state_key: StateKey, allowed_indices: np.ndarray) -> np.ndarray:
    """Return a deterministic, target-directed strategy for an unseen state."""
    strategy = np.zeros(len(ACTIONS), dtype=np.float32)
    allowed = {ACTIONS[index] for index in allowed_indices}

    target_mode, target_direction, _, bomb_state, _, _ = state_key

    preferred = []

    if target_mode == TARGET_ESCAPE and target_direction in MOVE_DELTAS:
        preferred.append(target_direction)

    if bomb_state == BOMB_ATTACK:
        preferred.append("BOMB")

    if target_direction in MOVE_DELTAS:
        preferred.append(target_direction)

    if target_mode == TARGET_BOMBING and target_direction == DIRECTION_HERE:
        preferred.append("BOMB")

    preferred.extend((*MOVE_DELTAS.keys(), "WAIT", "BOMB"))

    selected = next(action for action in preferred if action in allowed)
    strategy[ACTIONS.index(selected)] = 1.0

    return strategy


def solve_solo_action_policy(q_values: np.ndarray, visits: np.ndarray, state_key: StateKey, allowed_indices: np.ndarray):
    """Return a greedy policy while treating untried actions as neutral."""
    observed_indices = allowed_indices[visits[allowed_indices] > 0]

    if len(observed_indices) == 0:
        strategy = fallback_strategy(state_key, allowed_indices)
        return strategy, 0.0, "fallback"

    candidate_values = np.asarray(q_values[allowed_indices], dtype=np.float64).copy()
    candidate_values[visits[allowed_indices] == 0] = 0.0

    best_value = float(np.max(candidate_values))
    best_indices = allowed_indices[np.isclose(candidate_values, best_value)]

    strategy = np.zeros(len(ACTIONS), dtype=np.float32)
    strategy[best_indices] = 1.0 / len(best_indices)

    return strategy, best_value, "solo"


def nash_matrix_is_ready(visits: np.ndarray) -> bool:
    """Require useful coverage before trusting a minimax equilibrium."""
    observed = visits > 0

    return bool(
        visits.sum() >= MIN_NASH_TOTAL_VISITS
        and observed.mean() >= MIN_NASH_COVERAGE
        and np.all(np.any(observed, axis=1))
        and np.all(np.any(observed, axis=0))
    )


def complete_sparse_matrix(matrix: np.ndarray, visits: np.ndarray) -> np.ndarray:
    """Fill missing joint actions with the visit-weighted mean of their row."""
    completed = np.asarray(matrix, dtype=np.float64).copy()

    for row in range(completed.shape[0]):
        observed = visits[row] > 0
        weights = visits[row, observed]
        row_mean = float(np.average(completed[row, observed], weights=weights))
        completed[row, ~observed] = row_mean

    return completed


def q_matrix_for_state(q_table: dict[StateKey, np.ndarray], state_key: StateKey, dtype=np.float32)-> np.ndarray:
    """Return the Nash Q-matrix for a state."""
    values = q_table.get(state_key)

    if values is None:
        return np.zeros((len(ACTIONS), len(ACTIONS)), dtype=dtype)
...
def solve_zero_sum_game(q_matrix: np.ndarray)-> tuple[np.ndarray, np.ndarray, float]:
    ...
    if not np.any(matrix):
        row_strategy = np.ones(matrix.shape[0], dtype=np.float32) / matrix.shape[0]
        column_strategy = np.ones(matrix.shape[1], dtype=np.float32) / matrix.shape[1]

        return row_strategy, column_strategy, 0.0
...
def solve_single_agent_state(q_matrix: np.ndarray)-> tuple[np.ndarray, float]:
    ...
    strategy = np.zeros(matrix.shape[0], dtype=np.float32)
...
def load_model(path: Path) -> dict:
    """Load and validate all value and visit tables of a v4 model."""
    ...
    result = {
        "q_table": validate_table(payload.get("q_table"), (len(ACTIONS), len(ACTIONS)), np.float32, "joint Q"),
        "visit_table": validate_table(payload.get("visit_table"), (len(ACTIONS), len(ACTIONS)), np.int32, "joint visit", nonnegative=True),
        "solo_q_table": validate_table(payload.get("solo_q_table"), (len(ACTIONS),), np.float32, "solo Q"),
        "solo_visit_table": validate_table(payload.get("solo_visit_table"), (len(ACTIONS),), np.int32, "solo visit", nonnegative=True),
    }

    training_rounds = payload.get("training_rounds", 0)

    if not isinstance(training_rounds, int) or training_rounds < 0:
        raise ValueError("Invalid number of completed training rounds.")

    result["training_rounds"] = training_rounds

    return result


def validate_table(raw_table, expected_shape: tuple[int, ...], dtype, label: str, nonnegative: bool = False) -> dict:
    """Validate one state-indexed array table."""
    if not isinstance(raw_table, dict):
        raise ValueError(f"The saved payload contains no valid {label} table.")
    ...
    for key, raw_values in raw_table.items():
        ...
        if values.shape != expected_shape:
            raise ValueError(f"Invalid {label} shape {values.shape} for state {key!r}.")

        if not np.all(np.isfinite(values)):
            raise ValueError(f"Invalid values in the {label} table for state {key!r}.")

        if nonnegative and np.any(values < 0):
            raise ValueError(f"Negative counts in the {label} table for state {key!r}.")
...
def load_q_table(path: Path) -> dict[StateKey, np.ndarray]:
    """Compatibility helper returning the joint Q-table."""
    return load_model(path)["q_table"]


def available_actions(game_state: dict) -> list[str]:
    """Return the complete action space; learning must reject bad actions."""
    del game_state
    return list(ACTIONS)


def game_state_for_actor(game_state: dict, actor: tuple) -> dict:
    """Return a shallow game-state view with one opponent acting as self."""
    actor_name = actor[0]
    actor_state = dict(game_state)
    actor_state["self"] = actor
    actor_state["others"] = [
        game_state["self"],
        *(other for other in game_state["others"] if other[0] != actor_name),
    ]
