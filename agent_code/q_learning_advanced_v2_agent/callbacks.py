...
# 4 direction to nearest useful bombing position + 1 whether waiting is safe + 1 whether moving into at least one position is safe
N_FEATURES = 1 + 4 + 4 + 4 + 1 + 4 + 1 + 1 + 4 + 1 + 1
...
def state_to_features(game_state: dict) -> np.ndarray | None:
    ...
    features[24] = wait_is_safe
    features[25] = safe_move_exists
