...
# position + 4 route-preserving movement actions + 1 useless-bomb indicator
N_FEATURES = 32
...
def bomb_hits_no_crate(game_state: dict) -> bool:
    """Return whether a currently placeable bomb would hit no crate."""
...
def state_to_features(game_state: dict) -> np.ndarray | None:
    ...
    # A zero-valued quality feature cannot express that bombing here is bad.
    # This complementary feature lets the BOMB row learn that explicitly.
    features[31] = float(bomb_hits_no_crate(game_state))
