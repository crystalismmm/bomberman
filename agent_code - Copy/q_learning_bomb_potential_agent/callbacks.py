...
# 4 direction to nearest useful bombing position + 1 bomb available
N_FEATURES = 1 + 4 + 4 + 4 + 1 + 4 + 1 + 1 + 4 + 1
...
def can_escape_from_position(game_state: dict, bomb_position: tuple[int, int]) -> bool:
    """Check whether the agent can escape after placing a bomb at a given position."""

    simulated_bombs = list(game_state["bombs"]) + [(bomb_position, BOMB_TIMER - 1)]
...
def bomb_potential(crates: int, distance: int, can_escape: bool) -> float:
    """Score a bombing position based on destruction, accessibility and survival."""

    return crates*(BOMB_DECAY**distance) if can_escape else 0.0

def best_bombing_position(game_state: np.ndarray, max_distance=8):
    """
    Find the best nearby bombing location.
    """
    ...
    queue = deque([(start, 0, None)])   # position, distance, first_move
    visited = {start}
    ...
    while queue:
        ...
        for action, (dx, dy) in MOVE_DELTAS.items():
            ...
            if (
                ...
                visited.add(neighbor)
                queue.append(
                    (neighbor, distance+1, first_move or action) # clancker
                )
...
def state_to_features(game_state: dict) -> np.ndarray | None:
    ...
    # Features for the direction to the best bombing position
    best_bombing_direction, best_bombing_score  = best_bombing_position(game_state)


    for idx, move in enumerate(MOVE_DELTAS.keys()):
        if move == best_bombing_direction:
            features[20 + idx] = 1.0

    # Bomb availability
    features[24] = float(game_state["self"][2] > 0)
