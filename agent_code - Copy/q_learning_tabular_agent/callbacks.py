...
def bomb_status(game_state: dict) -> str:
    ...
    if not can_escape_after_bomb(game_state):
        return BOMB_UNSAFE
    if count_crates_in_blast(game_state["field"], position) == 0:
        return BOMB_USELESS
    return BOMB_GOOD
...
def nearest_safe_bombing_position(
    ...
    """Return the nearest useful and escapable bombing position.

    Distance is the primary criterion; among equally near positions, a tile
    hitting more crates is preferred.  No continuous potential is used.
    """
    if distances is None or parents is None:
        distances, parents = reachable_positions(game_state)
    ...
    for position, distance in sorted(distances.items(), key=lambda item: item[1]):
        ...
        candidates.append((distance, -crate_count, position))

    if not candidates:
        return None, None, None

    distance, _, target = min(candidates)
    direction = _first_direction(game_state["self"][3], target, parents)
    return direction, target, distance
